...
impl ErrorClassification for VenueError {
    fn category(&self) -> ErrorCategory {
        match self {
            ...
            VenueError::VenueSpecific { code, .. } => {
                ...
                // Note: Specific codes must come before ranges to avoid being caught by ranges
                match code {
                    // Rate limiting (must be before ranges)
                    -1015 => ErrorCategory::ResourceExhausted,
                    // Timestamp issues - transient (must be before ranges)
                    -1021 | -1022 => ErrorCategory::Transient,
                    // Order issues
                    -2010 | -2011 => ErrorCategory::Permanent,
                    // Unknown errors - may be transient (-1000 to -1099, excluding specific codes above)
                    -1099..=-1000 => ErrorCategory::Internal,
                    // Request issues - usually permanent (bad params) (-1100 to -1199)
    ...
    fn suggested_retry_delay(&self) -> Option<Duration> {
        match self {
            ...
            VenueError::VenueSpecific { code, .. } => {
                match code {
                    -1015 => Some(Duration::from_secs(60)), // Rate limit
                    -1021 | -1022 => Some(Duration::from_millis(100)), // Timestamp
                    _ => None,
                }
            }
            _ => None,
...
mod tests {
    ...
    fn test_error_code_extraction() {
        let err = VenueError::order_rejected_with_code("Bad order", -2010);
        assert_eq!(err.error_code(), Some(-2010));
