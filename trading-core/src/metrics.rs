...
lazy_static! {
    ...
    /// Current IPC connection status (1=connected, 0=disconnected)
    pub static ref IPC_CONNECTION_STATUS: IntGauge = IntGauge::new(
        "trading_ipc_connection_status",
        "Current IPC connection status (1=connected, 0=disconnected)"
    ).expect("Failed to create ipc_connection_status metric");

    /// Total IPC reconnection attempts
    pub static ref IPC_RECONNECTS_TOTAL: IntCounter = IntCounter::new(
        "trading_ipc_reconnects_total",
        "Total number of IPC reconnection attempts"
    ).expect("Failed to create ipc_reconnects_total metric");
...
pub fn register_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // IPC connection metrics
    REGISTRY.register(Box::new(IPC_CONNECTION_STATUS.clone()))?;
    REGISTRY.register(Box::new(IPC_RECONNECTS_TOTAL.clone()))?;
